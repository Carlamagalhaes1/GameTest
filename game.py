...
class Enemy:
    ...
    def update(self, dt):
        ...
        if self.moving:
            ...
            if self.t >= 1:
                self.t, self.moving = 1, False
                self.cool, self.sprite.state = 0.5, "idle"
            self.x = self.src[0] * (1 - self.t) + self.dst[0] * self.t
            self.y = self.src[1] * (1 - self.t) + self.dst[1] * self.t
        elif self.cool <= 0:
            self.step()
...
class Button:
    def __init__(self, text, y):
        self.text, self.r = text, Rect(WIDTH // 2 - 120, y, 240, 60)
    def draw(self):
        screen.draw.filled_rect(self.r, (200,200,255))
        screen.draw.rect(self.r, (0,0,100))
...
class SmallButton:
    def __init__(self, text, x, y, w=120, h=36):
        ...
        self.r = Rect(x, y, w, h)
    def draw(self):
        screen.draw.filled_rect(self.r, (210,210,230))
        screen.draw.rect(self.r, (30,30,60))
...
def draw():
    ...
    if game_state == STATE_MENU:
        ...
        screen.draw.text("Mini Roguelike", center=(WIDTH//2,120), fontsize=48, color="white")
        ...
        for y, row in enumerate(RAW_MAP):
            for x, c in enumerate(row):
                ...
                tile_rect = Rect(OFFSET_X + x*TILE, OFFSET_Y + y*TILE, TILE, TILE)
                screen.draw.filled_rect(tile_rect, col)
                if (x, y) == (DOOR_CX, DOOR_CY):
                    screen.draw.filled_rect(tile_rect.inflate(-8, -4), (40,160,40))
                    screen.draw.filled_circle((tile_rect.right-12, tile_rect.centery), 3, (230,230,120))
...
def on_key_down(key):
    ...
    if game_state == STATE_GAME:
        if key == keys.ESCAPE: set_menu()
            ...
            music_on = not music_on; toggle_music()
            game_btns[0].text = f"Música {'ON' if music_on else 'OFF'}"
        elif key == keys.N: 
            sfx_on = not sfx_on
...
def on_mouse_down(pos):
    ...
    if game_state == STATE_MENU:
        if   btns[0].hit(pos): set_game()
            ...
            music_on = not music_on; toggle_music()
            btns[1].text = f"Musica {'ON' if music_on else 'OFF'}"
...
def toggle_music():
    if music_on:
        music.play("bgm"); music.set_volume(0.6)
    else:
        music.stop()
