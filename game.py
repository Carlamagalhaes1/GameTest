...
btns = [Button("Start Game", 250), Button("Toggle Music", 350), Button("Quit", 450)]
    ...
    if game_state == STATE_MENU:
        ...
        for y, row in enumerate(RAW_MAP):
            for x, c in enumerate(row):
                ...
                screen.draw.filled_rect(Rect(OFFSET_X + x * TILE, OFFSET_Y + y * TILE, TILE, TILE), col)
...
def toggle_music():
    if music_on:
        print("Tentando tocar música: bgm")
        try:
            music.play("bgm")     # procura bgm.ogg, bgm.wav ou bgm.mp3 em /music
            music.set_volume(0.6) # volume entre 0.0 e 1.0 (opcional)
            print(" Música tocando em loop")
